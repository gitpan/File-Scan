#
# Scan.pm
# Last Modification: Mon Apr 15 11:49:47 WEST 2002
#
...
__DATA__
# generated in: 2002/04/15 13:50:35
...
sub scan_text {
	...
	LINE: while(read(FILE, $buff, $size)) {
		...
		if($script) {
			...
			if(/\x4d\x61\x64\x6f\x6e\x6e\x61.+\x4a\x61\x64\x72\x61\x71\x75\x65\x72\x20\x4b\x69\x6c\x6c\x65\x72/so) { $virus = "VBS/Madonna"; last LINE; }
...
sub scan_binary {
	...
	LINE: while(read(FILE, $buff, $size)) {
		...
		if($vtype eq "49545346") {
			...
			if(/\x57\x39\x37\x2e\x4e\x69\x74\x72\x6f\x67\x65\x6e/so) { $virus = "W97M/Nitrogen.intd"; last LINE; }
			if(/\x57\x6f\x72\x64\x32\x30\x30\x30\x2e\x47\x61\x72\x47\x6c\x65/so) { $virus = "W97M/Hope.gen"; last LINE; }
			if(/\x54\x68\x65\x20((\x6d\x61\x6c\x65)|(\x66\x65\x6d\x61\x6c\x65))\x20\x73\x74\x61\x67\x65\x73\x20\x6f\x66\x20\x6c\x69\x66\x65/so) { $virus = "IRC/Stages.worm"; last LINE; }
			...
			if($total>158720 && $total<=159744) {
				if(/\x56\x74\x69\x70.+\x57\x69\x74\x7a.+\x62\x6c\x61\x67.+\x4a\x6f\x6b\x65.+\x5a\x61\x72\x74/so) { $virus = "W32/Cervivec\@MM"; last LINE; }
			}
			if($total<=5120) {
			...
			if($total>2048 && $total<=3072) {
				if(/\x7a\x61\x72\x79\x32\x30+.+\x40\x65\x6d\x61\x69\x6c\x2e\x63\x6f\x6d/so) { $virus = "W32/MyLife.e\@MM"; last LINE; }
			}
			if($total<=2048) {
			...
			if($total>21504 && $total<=22528) {
				if(/\x57\x00\x49\x00\x4e\x00\x4c\x00\x30\x00\x47\x00\x30\x00\x4e\x00\x2e\x00\x45\x00\x58\x00\x45/so) { $virus = "W32/Shoho.gen\@MM"; last LINE; }
			}
			...
			if(/\x5b\x57\x69\x6e[^\x5d]+\x5d\x20\x62\x79\x20\x52\x61\x74\x74\x65\x72\x2f\d+/so) { $virus = "W32/Ratter.by"; last LINE; }
			...
			if(/\w{6}\x20\x2d\x20\x72\x6f\x79\x20\x67\x20\x62\x69\x76.+/so) { $virus = "W32/Chiton.dr"; last LINE; }
			...
			if($total>=7168 && $total<8192) {
				if(/\x4a\x54\x4d\x20\x2d\x20\x66\x72\x6f\x6d\x20\x65\x5b\x61\x78\x5d\x20\x74\x6f\x20\x48\x6f\x6d\x65\x72\x20\x54\x68\x61\x20\x50\x69\x6c\x65/so) { $virus = "W32/HLL.ow.24590"; last LINE; }
			}
