	...
	LINE: while(read(FILE, $buff, $size)) {
		...
		if($type == 1) {
			...
			if($subtype == 1) {
				...
				if($total==16384) {
					/[\x7c\x4c][\x20\xfc]\x46\x3c?\x52\x4f[\x57\x4d][\x3a\x2b\x12].{12,35}\x54/s and $virus = "W32/Netsky\@MM", last LINE;
				}
				if($total==17408) {
				...
				if($total==39936) {
					/\x7b\x3f\x01\xff\x1f\xfb\xff\x69\x70\x68\x6c\x70\x61\x70\x69\x2e\x64\x6c\x6c\x27\x96\x30\x07\x77\x2c\x61\x0e\xee\xba\x51\x09\xff\xff\xff\xff\x99/s and $virus = "New Malware.b", last LINE;
				}
				if($total<=4096) {
