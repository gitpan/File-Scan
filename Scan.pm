#
# Scan.pm
# Last Modification: Thu Mar 11 17:34:53 WET 2004
#
...
__DATA__
# generated in: 2004/03/11 17:44:08
...
sub scan_binary {
	...
	LINE: while(read(FILE, $buff, $size)) {
		...
		if($type == 1) {
			...
			if($subtype == 1) {
				if($total==7168) {
					/\x31\x35\x31.{0,4}\x2e\x32\x30\x30\x2e\x33\x39/s and $virus = "W32/Bagle.j\@MM", last LINE;
				}
			} elsif($subtype == 2) {
				if($total==1024) {
				...
				if($total==2048 || $total==5120) {
					/\x88\x02\x00\x00.{4}\x8b\x44\x24\x04\x83\xc4\x2a\x8d\x90\x34\x00\x00\x00\x83\xc4\x08\x6a\x10\x8b\xd8\x66\x05\x2d\x00\x50\x52\x6a\x00\x8b\x1b\xff\x13\x6a\xff\xff\x53\x0c\x45\x52\x52\x4f\x52\x21\x00\x43\x6f\x72\x72\x75\x70\x74\x20\x44\x61\x74\x61\x21\x00\xb8\x00.\x41\x00\x68..\x40\x00\x64\xff\x35\x00/s and $virus = "W32/Netsky.c\@MM", last LINE;
				}
