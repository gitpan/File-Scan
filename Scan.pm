#
# Scan.pm
# Last Modification: Tue Jan 27 14:59:36 WET 2004
#
...
__DATA__
# generated in: 2004/01/27 15:32:37
		...
		if($type == 1) {
			...
			if($subtype == 1) {
				...
				if($total==12288 || $total==28672) {
					/\x4d\x41\x49\x4c\x20\x46\x52\x78?\x4f.\x3a.\x3c.{17,26}\x52\x43\x50\x39?\x54\x20.\x4f.{3,4}\x44\x41.{3,6}\x51\x55\x49.{4,9}\x59/s and $virus = "W32/Dumaru\@MM", last LINE;
				}
				if($total==7168) {
				...
				if($total==1024) {
					...
					/\x00\x40\x00\x00\x38\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x62\x65\x61\x67\x6c\x65\x00\x00\x86\x23\x00\x00\x00\x10\x00\x00\x00\x24\x00/s and $virus = "W32/Bagle\@MM", last LINE;
				...
				if($total==6144) {
					/\x3b\x5b\xf5\xff\xff\x5f\x41\xcd\xf9\x28\x5a\x8f\xc7\x28\x73\x79\x6e\x63\x2e\x63\x2c\x76\x20\x30\x2e\x31\x20\x32\x30\x30\x34\xfd\x23\xdb\x6f\x93\x31\x2f\x78\x78\x20\x02\x3a\x20\x61\x6e\x64\x79\x29\x00\x7b\xbb/s and $virus = "W32/Mydoom\@MM", last LINE;
				}
				if($total==37888) {
				...
				if($total>=8192 && $total<=20480) {
					/\x51.{1,2}\x54.{1,6}\x02.{9,34}\x41\x54\x41.{0,5}\x52.{0,5}\x43.{1,6}\x54.{0,5}\x4f\x3a.\x0e\x4d\x41.{0,4}\x49\x4c.{3,8}\x10.{2,10}\x6c/s and $virus = "W32/Mimail\@MM", last LINE;
				}
				if($total<=4096) {
