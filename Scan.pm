	...
	LINE: while(read(FILE, $buff, $size)) {
		...
		if($type == 1) {
			...
			if($subtype == 1) {
				...
				if($total>=8192 && $total<=11264) {
					/\x51\x55\x49\x54.{1,6}\x02\x2e\x0d.{7,17}\x44\x41\x54\x41.{0,5}\x52.{0,5}\x43.{1,6}\x54.{0,5}\x4f\x3a.\x0e\x4d\x41.{0,4}\x49\x4c.{1,5}\x52\x4f.{0,4}\x4d\x10.{2,10}\x6c/s and $virus = "W32/Mimail\@MM", last LINE;
				}
				if($total==102400) {
